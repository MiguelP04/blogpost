...
export default function UserAlbums({ albums, selectedAlbumId, setSelectedAlbumId, photos }) {
  ...
  return (
    <div>
      {!selectedAlbumId && (
        <>
          <h1 className="text-2xl font-bold mb-6 text-indigo-700 text-center">Álbumes</h1>
          {albums.length === 0 ? (
              ...
              <div className="w-full max-w-6xl">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                  {albums.map((album) => (
                    <div
                      key={album.id}
                      className={`cursor-pointer rounded-2xl overflow-hidden shadow-lg transform transition hover:-translate-y-1 hover:shadow-2xl border ${selectedAlbumId === album.id ? "ring-4 ring-indigo-100" : "border-gray-100"}`}
                    ...
                    >
                      ...
                      <div className="p-4 bg-white">
                        <p className="text-sm text-gray-500 line-clamp-2">{album.title}</p>
                      </div>
                    </div>
                  ))}
                </div>
        ...
        <div className="w-full flex justify-center mt-6">
          <div className="w-full max-w-6xl">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-semibold text-indigo-700">Fotos del álbum</h2>
              <button
            ...
            {photos.length === 0 ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
                {photos.slice(0, 12).map((photo) => (
                  <div key={photo.id} className="rounded-2xl overflow-hidden shadow-lg transform hover:scale-105 transition relative bg-white">
                    <button
